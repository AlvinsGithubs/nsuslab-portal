...
export interface SectionContent {
  title: string;
  description?: string; // Pure text with \n for line breaks
  accordionItems?: AccordionItem[];
  images: string[];
}

export interface WsopPageData {
  hero: {
    title: string;
    subtitle: string;
    bgImage: string;
  ...
  sections: {
    ...
    qualify: SectionContent;
    history: SectionContent;
    prizes: SectionContent;
    academy: SectionContent;
    champions: SectionContent;
    tips: SectionContent;
    partnership: SectionContent;
    whyPlay: SectionContent;
    ready: SectionContent;
  };
}

// --- Data ---
export const wsopData: WsopPageData = {
  hero: {
    title: "WSOP",
    subtitle: "The World Series of Poker",
    bgImage:
  ...
  sections: {
    intro: {
      ...
      description: `World Series of Poker (WSOP)는 라이브 및 온라인 포커 토너먼트의 정점입니다. 1970년 창설 이래 WSOP는 매년 수만 명의 플레이어를 끌어들이는 세계적인 현상으로 성장했습니다. 전설적인 메인 이벤트부터 소규모 전문 토너먼트에 이르기까지 WSOP는 모든 유형의 포커 플레이어에게 무언가를 제공합니다.
포커를 배우고자 하는 분들을 위해, 초보자와 기술 향상을 원하는 이들을 위한 포괄적인 튜토리얼과 자료가 마련되어 있습니다.
이 시리즈는 궁극의 포커 트로피로 여겨지는 탐나는 WSOP 브레이슬릿을 수여합니다. WSOP 브레이슬릿을 획득하는 것은 상금 그 이상입니다. 이는 탁월함의 상징이며 모든 플레이어의 포커 경력에서 핵심적인 이정표입니다.`,
      images: [
    ...
    events: {
      ...
      description:
        "WSOP는 모든 기술 수준과 뱅크롤의 플레이어들을 만족시키기 위해 설계된 다양한 이벤트 라인업을 특징으로 합니다. 다음은 WSOP 일정의 몇 가지 하이라이트입니다.",
      accordionItems: [
        {
    ...
    qualify: {
      ...
      description:
        "GGPoker 덕분에 WSOP 참가 자격을 얻는 것이 그 어느 때보다 쉬워졌습니다. 다음은 참가권을 확보하는 가장 인기 있는 방법입니다.",
      accordionItems: [
        {
    ...
    history: {
      ...
      description: `WSOP는 1970년 베니 비니언(Benny Binion)에 의해 창설되었으며, 원래 라스베이거스의 호스슈 카지노(Horseshoe Casino)에서 개최되었습니다. 첫 번째 이벤트는 초청 캐시 게임이었고, 조니 모스(Johnny Moss)가 동료들에 의해 최초의 챔피언으로 뽑혔습니다. 이는 경쟁 포커의 초석이 될 시리즈의 시작을 알렸습니다.
1970년대와 1980년대 내내 WSOP는 상금 풀과 플레이어 참여가 증가함에 따라 더 넓은 청중을 끌어들이기 시작했습니다. 도일 브런슨(Doyle Brunson), 스튜 언가(Stu Ungar), 필 헬뮤트(Phil Hellmut)와 같은 상징적인 챔피언들이 이 시기에 등장하여 각각 여러 개의 브레이슬릿을 획득하며 그들의 유산을 확고히 했습니다.
2003년, WSOP는 크리스 머니메이커(Chris Moneymaker)가 메인 이벤트에서 믿기 힘든 우승을 차지한 덕분에 폭발적인 인기를 얻었습니다. 39달러 온라인 새틀라이트를 통해 참가 자격을 얻은 그의 승리는 전 세계적인 포커 붐을 일으켰고, 누구나 포커의 위대함을 성취할 수 있는 토너먼트로서의 WSOP의 명성을 공고히 했습니다.
시리즈는 100만 달러 바이인을 특징으로 하며 자선기금으로 수백만 달러를 모금한 Big One for One Drop과 같은 새로운 이벤트를 도입하며 계속 진화했습니다. 2007년 WSOP는 World Series of Poker Europe (WSOPE)의 론칭과 함께 국제적으로 확장하여 WSOP의 흥분을 세계 무대로 가져왔습니다.
최근에는 WSOP와 GGPoker 간의 파트너십을 통해 2020년 사상 최초의 WSOP 온라인 시리즈가 탄생했습니다. 이 획기적인 계획은 역사상 가장 큰 온라인 포커 상금 풀을 포함하여 참가 기록을 세웠습니다. WSOP는 역동적이고 끊임없이 진화하는 시리즈로 남아 있으며, 전 세계 경쟁 포커의 기준을 계속 설정하고 있습니다.`,
      images: [
    ...
    prizes: {
      ...
      description:
        "WSOP 이벤트에서 우승하는 것은 현금 이상의 의미가 있습니다. 각 토너먼트는 포커 성취의 궁극적인 상징이 된, 모두가 탐내는 WSOP 브레이슬릿을 수여합니다. 이 브레이슬릿은 복잡한 디자인을 특징으로 하며 금과 같은 귀금속으로 세심하게 제작됩니다. 수년에 걸쳐 브레이슬릿 디자인은 진화했으며, 최근 버전에는 루비, 다이아몬드 및 기타 보석이 포함되어 포커 세계에서의 그 위상을 반영합니다.",
      accordionItems: [
        {
          title: "Prize Pools",
          description:
    ...
    academy: {
      title: "Poker Academy",
      description: `GGPoker 스쿨은 모든 수준의 포커 애호가들을 위한 지식의 보고입니다. 막 시작했거나 고급 전략을 다듬고 싶다면, 아카데미는 포커 기술을 향상시키는 데 도움이 되는 풍부한 자료를 제공합니다.
아카데미가 차별화되는 점은 최고의 포커 플레이어와 코치들의 전문적인 조언입니다. 기본을 마스터하고 싶거나 경쟁 우위를 확보하고자 한다면, GGPoker 스쿨은 포커에 관한 모든 것을 위한 최고의 자료입니다.`,
      images: [
    ...
    champions: {
      ...
      description: `World Series of Poker는 포커 역사에 지울 수 없는 족적을 남긴 챔피언들을 배출해 온 풍부한 유산을 가지고 있습니다. 전설적인 도일 브런슨과 스튜 언가부터 다니엘 네그라누, 필 헬뮤트와 같은 현대의 아이콘에 이르기까지 WSOP 메인 이벤트 우승은 포커 성취의 정점으로 간주됩니다.
메인 이벤트의 상금 풀은 종종 천문학적인 액수에 도달하여, 포커 기술과 인내심의 궁극적인 시험대가 됩니다. 과거 챔피언들의 이야기, 그들의 전략, 그리고 승리는 새로운 세대의 플레이어들에게 계속 영감을 주며, WSOP의 위상을 최고의 포커 챔피언십으로 공고히 하고 있습니다.`,
      images: [
    ...
    tips: {
      ...
      description:
        "WSOP에 처음 참가하는 것은 잊을 수 없는 경험이 될 수 있습니다. 다음은 준비에 도움이 되는 몇 가지 팁입니다.",
      accordionItems: [
        {
    ...
    partnership: {
      ...
      description:
        "GGPoker는 WSOP 경험의 핵심적인 부분이 되었습니다. WSOP의 공식 파트너로서 GGPoker는 다음을 통해 플레이어들에게 시리즈에 대한 독보적인 접근성을 제공합니다.",
      accordionItems: [
        {
    ...
    whyPlay: {
      ...
      description:
        "GGPoker가 WSOP 여정을 시작하기에 가장 좋은 장소인 몇 가지 이유가 있습니다.",
      accordionItems: [
        {
          title: "Accessibility",
          description:
    ...
    ready: {
      ...
      description: `World Series of Poker는 단순한 토너먼트 시리즈가 아닙니다. 이는 우리 모두가 사랑하는 게임의 축제입니다. 수십 년에 걸쳐 WSOP는 전 세계 포커 커뮤니티를 형성하고 다양한 배경의 플레이어들 간의 연결을 촉진하며 포커를 합법적인 경쟁 스포츠로 확립했습니다.
이 시리즈는 또한 포커 전략과 문화에 지대한 영향을 미쳤습니다. 새로운 토너먼트 형식 도입과 온라인 포커 수용과 같은 WSOP의 혁신은 게임을 글로벌 관객에게 접근 가능하게 유지하면서 진화시키는 데 도움을 주었습니다.
브레이슬릿을 쫓든, 뱅크롤을 쌓든, 아니면 단순히 높은 스테이크의 경쟁의 스릴을 경험하고 싶든, WSOP는 모두를 위한 무언가를 갖추고 있습니다.
GGPoker는 WSOP로 가는 관문입니다. 지금 바로 WSOP 새틀라이트를 탐색하고, 온라인 게임에서 기술을 연마하며, 포커 영광의 순간을 준비하며 여정을 시작하세요. 지금 가입하여 WSOP 역사의 일부가 되는 첫걸음을 내딛으세요.`,
      images: [
