...
const CultureSection: React.FC = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="w-full bg-white py-12 lg:py-24 px-4 md:px-8 text-black"
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {cultureData.map((card) => (
            <a
              key={card.id}
              href={`#${card.linkTo}`}
              className="animated-item group flex flex-col bg-white overflow-hidden rounded-2xl border border-gray-200"
            >
              <div className="w-full md:aspect-[4/3] overflow-hidden">
                <img
                  src={card.image}
                  alt={card.title}
              ...
              <div className="p-8 flex flex-col flex-grow">
                ...
                <div className="inline-flex items-center pt-4">
                  <p className="text-black font-bold group-hover:text-nsus-blue">
                    {card.linkText}
                  </p>
