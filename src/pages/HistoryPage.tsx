import React, { useState, useEffect, useRef, useContext } from "react"; // ✅ useState, useEffect 추가
...
const HistoryPage: React.FC = () => {
  ...
  return (
    <div>
      {/* Hero Section (변경 없음) */}
      <section className="h-[100svh] flex flex-col justify-center items-center relative text-center px-4 text-white overflow-hidden">
        <video
          autoPlay
          loop
          muted
        ...
        <div
          ...
          className="absolute bottom-10 animate-bounce z-20 cursor-pointer"
        >
              ...
              historyData.map(({ year, events }) => (
                <div key={year} className="flex items-start">
                  <div className="w-24 text-left">
                    <p className="text-xl font-bold text-nsus-gray-900 sticky top-24">
                      {year}
                    </p>
                  </div>
                  <div className="flex-1 pl-8">
                    <div className="border-t border-nsus-gray-200">
                      {events.map((event, index) => (
                        <div
                          key={index}
                          className="flex items-baseline space-x-8 pt-5"
                        >
                          <p className="w-12 text-sm font-bold text-nsus-gray-700 uppercase">
                            {event.month}
                          </p>
                          <p className="text-nsus-gray-700">
                            {event.description}
                          </p>
                        </div>
                      ))}
