...
const JobListItem: React.FC<{ job: Job }> = ({ job }) => {
  ...
  return (
    <a
      ...
      className="block py-6 border-b border-nsus-gray-200 group"
    >
      <h4 className="font-bold text-xl text-nsus-gray-900 group-hover:text-nsus-blue transition-colors">
        {job.title}
      </h4>
...
const CareersPage: React.FC = () => {
  const createInitialCheckboxState = (
    options: readonly string[]
  ): JobFilterCheckboxState =>
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const jobResponse = await contentfulClient.getEntries({
          content_type: "job",
  ...
  const filteredJobs = useMemo(() => {
    const getSelected = (state: JobFilterCheckboxState) =>
      ...
      const employmentTypeMatch =
        ...
        (job.employmentType &&
          selectedEmploymentTypes.some(
            (type) => type === "정규직" && job.employmentType === "Full-time"
          ));
      ...
      const locationMatch =
        ...
        (job.location &&
          selectedLocations.some(
            (loc) =>
              loc === "NSUSLAB KOREA" &&
              (job.location === "Seoul" || job.location === "Bundang")
          ));
    ...
    return (
      <div>
        <div className="py-6 border-b border-nsus-gray-200">
          <h4 className="font-bold text-xl text-nsus-gray-900">
            인재 Pool 등록
          </h4>
              ...
              <div className="mt-8 divide-y divide-nsus-gray-200">
                <FilterAccordion
                  ...
                  onChange={(option, checked) =>
                    handleFilterChange("jobGroup", option, checked)
                  }
                />
                <FilterAccordion
                  title="경력사항"
                  ...
                  onChange={(option, checked) =>
                    handleFilterChange("careerLevel", option, checked)
                  }
                />
                <FilterAccordion
                  title="고용형태"
                  ...
                  onChange={(option, checked) =>
                    handleFilterChange("employmentType", option, checked)
                  }
                />
                <FilterAccordion
                  title="근무지"
                  ...
                  onChange={(option, checked) =>
                    handleFilterChange("location", option, checked)
                  }
                />
              </div>
            </div>
